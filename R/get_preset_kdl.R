...
get_preset_kdl <- function(path_hsi, bands = 15, outdir = NA, mat_args = NULL){
  fname <- substring(path_hsi, nchar(path_hsi)-12+1, nchar(path_hsi))
  hsi <- brick(path_hsi)
  kld_groups <- readr::read_csv("./indir/Misc/bands_clusters_kld.csv") %>%
     data.frame
  ...
  v <- cbind.data.frame(kld_groups, t(v)) %>%
    group_by(aa) %>% summarise_all(mean) %>%
    mutate_all(~ . /10000) %>% t
  v <- v[-1,]
  x <- hsi[[1:bands]]
